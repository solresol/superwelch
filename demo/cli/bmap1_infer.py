...
parser.add_argument("--model-database", required=True, help="SQLite database where train/bmap1.py ran")
...
cursor.execute("SELECT model FROM bmap1_storage")
row = cursor.fetchone()
if row is None:
    sys.exit(f"{args.model_database} does not contain a trained BMAP1 model")
...
while True:
    ...
    try:
        if sys.stdin.isatty():
            if current_experiment_id is None:
                prompt = "BMAP1-cli (no experiment): "
                c = WordCompleter(['new'])
            else:
                prompt = f"BMAP1-cli #{current_experiment_id}: "
                c = word_completer
    ...
    if current_experiment_id is not None and user_input.lower().strip().startswith("test"):
        ...
        print_formatted_text(FormattedText([('fg:green', f"BMAP1 test reported: {model_pretty_result} with probability of it being successful = {probs[1]}")]))
        cursor.execute("update real_experiments set welch_ttest_result = ?, welch_ttest_pvalue = ?, bmap1_result = ?, bmap1_pvalue = ? where real_experiment_id = ?",
