...
cursor.execute("""create table if not exists bmap1_training (
...
if not(args.force_retrain):
    cursor.execute("select last_trained from bmap1_training")
    row = cursor.fetchone()
    if row is not None:
        last_trained = row[0]
...
cursor.execute("delete from bmap1_training;")
cursor.execute("""INSERT OR REPLACE INTO bmap1_training
    (type_1_error_ratio, type_2_error_ratio, roc_auc, accuracy, last_trained)
    VALUES (?, ?, ?, ?, current_timestamp);
""", data_to_insert)
conn.commit()
                                       
rfc.fit(raw_data_dataframe[control_columns + experiment_columns],
             raw_data_dataframe.was_successful)

cursor.execute("""
CREATE TABLE IF NOT EXISTS bmap1_storage (
    model BLOB,
    saved_on TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);""")
cursor.execute("delete from bmap1_storage")


# Serialize the model
model_buffer = BytesIO()
joblib.dump(rfc, model_buffer)
...
insert_query = """
    ...
    VALUES (?, ?);
"""
cursor.execute("insert into bmap1_storage (model) values (?)", (serialized_model,))
conn.commit()
