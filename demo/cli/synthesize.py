...
parser.add_argument("--proportion-that-do-nothing", type=float, default=0.5, help="Proportion of experiments that do nothing")
parser.add_argument("--number-of-experiments", type=int, default=100000, help="Number of experiments")
parser.add_argument("--control-mean-low", type=int, default=3, help="Mean range for experimental group")
parser.add_argument("--control-mean-high", type=int, default=4, help="Mean range for experimental group")
parser.add_argument("--control-stddev-low", type=float, default=0.5, help="Low end of stddev for control population")
parser.add_argument("--control-stddev-high", type=float, default=5, help="High end of stddev for control population")
parser.add_argument("--experimental-mean-low", type=int, default=-5, help="Mean range for experimental group")
parser.add_argument("--experimental-mean-high", type=int, default=10, help="Mean range for experimental group")
parser.add_argument("--experimental-stddev-low", type=float, default=0.2, help="Low end of stddev for experimental group")
parser.add_argument("--experimental-stddev-high", type=float, default=10, help="High end of stddev for experimental group")
...
cursor.execute("""SELECT  control_group_size, experiment_group_size, proportion_that_do_nothing,
        control_mean_low, control_mean_high, control_stddev_low, control_stddev_high,
        experimental_mean_low, experimental_mean_high, experimental_stddev_low,
        experimental_stddev_high  FROM metadata LIMIT 1;""")
row = cursor.fetchone()
...
if row:
    ...
    cursor.execute(insert_query, (
        args.control_group_size, args.experiment_group_size, args.proportion_that_do_nothing,
        args.control_mean_low, args.control_mean_high, args.control_stddev_low, args.control_stddev_high,
        args.experimental_mean_low, args.experimental_mean_high, args.experimental_stddev_low, args.experimental_stddev_high
    ))
...
def generate_experiment(cursor):
    null_experiment = scipy.stats.uniform.rvs() <= args.proportion_that_do_nothing
    control_loc = scipy.stats.uniform.rvs(loc=args.control_mean_low,
                                            scale=args.control_mean_high - args.control_mean_low)
    control_scale = scipy.stats.uniform.rvs(loc=args.control_stddev_low,
                                            scale=args.control_stddev_high - args.control_stddev_low)
    if null_experiment:
        experiment_loc = control_loc
        experiment_scale = control_scale
    else:
        experiment_loc = scipy.stats.uniform.rvs(loc=args.experimental_mean_low,
                                                 scale=args.experimental_mean_high - args.experimental_mean_low)
        experiment_scale = scipy.stats.uniform.rvs(loc=args.experimental_stddev_low,
                                                   scale=args.experimental_stddev_high - args.experimental_stddev_low)
    control_values = numpy.sort(scipy.stats.norm.rvs(loc=control_loc, scale=control_scale, size=args.control_group_size))
    experiment_values = numpy.sort(scipy.stats.norm.rvs(loc=experiment_loc, scale=experiment_scale, size=args.experiment_group_size))
    cursor.execute(f"insert into synthetic_experiments (control_mean, control_stddev, experiment_mean, experiment_stddev, {insert_args}, was_successful) values (?,?,?,?,{insert_params},?)",
                   [control_loc, control_scale, experiment_loc, experiment_scale] + list(control_values) + list(experiment_values) + [not(null_experiment)])

iterator = range(args.number_of_experiments)
if args.progress:
    import tqdm
    iterator = tqdm.tqdm(iterator)
for i in iterator:
    generate_experiment(cursor)
    if i % 100 == 0:
        conn.commit()
